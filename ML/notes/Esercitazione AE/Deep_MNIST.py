...
"""
model = Sequential()
model.add(Dense(units=l2, activation='relu', input_dim=n))
...
ep = 10
batches = 32
exists = os.path.isfile('deep.hist');
if exists:
    with open('deep.hist','rb') as hist_file:
        history_dict = pickle.load(hist_file);
        autoencoder.load_weights('deep')
else:
    history = autoencoder.fit(X_train, X_train, epochs=ep, batch_size=batches,
                             validation_data = (X_test, X_test));
    history_dict = history.history;
    with open('deep.hist','wb') as hist_file:
        pickle.dump(history_dict, hist_file);
    autoencoder.save_weights('deep');
...
hist_plot(history_dict, 'Deep AE')
